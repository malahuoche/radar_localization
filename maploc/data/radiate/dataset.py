...
class RadiateDataModule(pl.LightningDataModule):
    default_cfg = {
        **MapLocDataset.default_cfg,
        "name": "radiate",
        # paths and fetch
        "data_dir": "/home/classlab2/radiate",
        "tiles_filename": "rdiatetiles.pkl",
        "splits": {
    ...
    def get_frame_data(self, date, drive, index):
        ...
        gps_timestamps = gps_path = (self.root / date / "GPS_IMU_Twist.txt")
        cam_timestamps = gps_path = (self.root / date / "zed_left.txt")

        _, R_cam_gps, t_cam_gps = self.calibrations[date]
        # Transform the GPS pose to the camera pose
        gps_path = (
            self.root / date / "GPS_IMU_Twist" / Path(index).with_suffix(".txt")
        )
