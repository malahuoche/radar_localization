...
from ..models.metrics import AngleError, LateralLongitudinalError, Location2DError,AngleRecall_rotation
...
#evaluate rotation
@torch.no_grad()
def evaluate_single_image_rotation(
    dataloader: torch.utils.data.DataLoader,
    model: GenericModule,
    num: Optional[int] = None,
    callback: Optional[Callable] = None,
    ...
    metrics = MetricCollection(model.model.metrics())
    metrics["yaw_error"] = AngleError("x")
    metrics[ "yaw_recall_2°"]: AngleRecall_rotation(2.0, "x")
    metrics[ "yaw_recall_5°"]: AngleRecall_rotation(5.0, "x")
    metrics = metrics.to(model.device)
...
def evaluate(
    ...
    **kwargs,
    ...
    checkpoint_path = Path("/home/classlab2/root/OrienterNet/experiments/OrienterNet_MGL_1206_1/last.ckpt")
    print(checkpoint_path)
    # model = GenericModule.load_from_checkpoint(
    #     checkpoint_path, cfg=cfg, find_best=not experiment.endswith(".ckpt")
    # )
    model = GenericModule.load_from_checkpoint(
        checkpoint_path, cfg=cfg, find_best=False
    )
    model = model.eval()
    ...
    dataset.setup()
    # output_dir = Path("/home/classlab2/root/OrienterNet/output_test_1102")
    ...
    loader = dataset.dataloader(split, shuffle=True, num_workers=num_workers)
    metrics = evaluate_single_image_rotation(loader, model, **kwargs)
