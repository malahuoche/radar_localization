...
def parse_gps_file(path, projection: Projection = None):
    ...
    with open(path, "r") as fid:
        roll, pitch, yaw, lat, lon = map(float, fid.read().split())
...
def get_camera_calibration(calib_dir):
    ...
    calibration = Calib(calib_path)
    #radar的变换矩阵
    T_radar_lidar =calibration.T_radar_lidar
    T_applanix_lidar  =calibration.T_applanix_lidar 
    # 计算雷达到Lidar的逆变换矩阵
    T_lidar_radar = np.linalg.inv(T_radar_lidar)

    # 计算Applanix到Lidar的逆变换矩阵
    T_lidar_applanix = np.linalg.inv(T_applanix_lidar)

    # 计算雷达到Applanix的变换矩阵
    T_radar_applanix = np.dot(T_lidar_applanix, T_lidar_radar)

    # 提取旋转矩阵和平移向量
    R_radar_gps = T_radar_applanix[:3, :3]
    t_radar_gps = T_radar_applanix[:3, 3]
    #直接读取参数
